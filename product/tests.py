...
class ProductTestCase(TestCase):
    ...
    def setUp(self):
        ...
        models.Product.objects.create(name="test2P", price=12000, real_price="10000$", description="test_description", category=self.category)

    def test_list_products(self):
        response = self.client.get("/api/v1/product/products/")
        self.assertEqual(response.status_code, 200)
        self.assertIn("test1", response.data[0]['name'])
        self.assertIn("test2P", response.data[1]['name'])
